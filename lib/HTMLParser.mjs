 ...
 */
export default function HTMLParser(address, fragmentContext, options) {
  /***
   * These are the parser's state variables
   */
  let chars = null;
  let numchars = 0; // Length of chars
  let nextchar = 0; // Index of next char
  let input_complete = false; // Becomes true when end() called.
  let scanner_skip_newline = false; // If previous char was CR
  let reentrant_invocations = 0;
  const saved_scanner_state = [];
  let leftovers = "";
  let first_batch = true;
  let paused = 0; // Becomes non-zero while loading scripts


  // Tokenizer state
  let tokenizer = data_state; // Current tokenizer state
  let return_state;
  let character_reference_code;
  let tagnamebuf = "";
  let lasttagname = ""; // holds the target end tag for text states
  const tempbuf = [];
  let attrnamebuf = "";
  let attrvaluebuf = "";
  const commentbuf = [];
  const doctypenamebuf = [];
  let doctypepublicbuf = [];
  let doctypesystembuf = [];
  const attributes = [];
  let is_end_tag = false;

  // Tree builder state
  let parser = initial_mode; // Current insertion mode
  let originalInsertionMode = null; // A saved insertion mode
  const templateInsertionModes = []; // Stack of template insertion modes.
  const stack = new HTMLParser.ElementStack(); // Stack of open elements
  const afe = new HTMLParser.ActiveFormattingElements(); // mis-nested tags
  const fragment = (fragmentContext !== undefined); // For innerHTML, etc.
  let head_element_pointer = null;
  let form_element_pointer = null;
  ...
  let frameset_ok = true;
  let force_quirks = false;
  let pending_table_text;
  let text_integration_mode; // XXX a spec bug workaround?

  // A single run of characters, buffered up to be sent to
  // the parser as a single string.
  const textrun = [];
  let textIncludesNUL = false;
  let ignore_linefeed = false;
