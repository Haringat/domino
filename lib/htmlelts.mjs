...
const htmlElements = {};
const htmlNameToImpl = Object.create(null);

export {
  htmlElements as elements
};

export function createElement(doc, localName, prefix) {
  const impl = htmlNameToImpl[localName] || HTMLUnknownElement;
  return new impl(doc, localName, prefix);
}

function define(spec, defaultConstructor = HTMLElement, tagList = htmlElements, tagNameToImpl = htmlNameToImpl) {
  return defineElement(spec, defaultConstructor, tagList, tagNameToImpl);
}

function URL(attr) {
  return {
    get: function() {
      const v = this._getattr(attr);
      if (v === null) { return ''; }
      const url = this.doc._resolve(v);
...
export const HTMLElement = define({
  superclass: Element,
 ...
 */
define({
  ctor: HTMLUnknownElement
});


const formAssociatedProps = {
