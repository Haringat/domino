import Parser from "./cssparser/css/Parser.mjs";
...
const NO_CHANGE = {}; // Private marker object
export default class CSSStyleDeclaration {
   ...
   */
  _serialize() {
    const styles = this._parsed;
    let s = "";

    for(const name in styles.property) {
      const value = styles.property[name];
      if (s) s += " ";
      s += name + ": " + value;
      if (value) {
        s += " !" + value;
  ...
  setProperty(property, value, priority) {
    ...
    if (value === NO_CHANGE) {
      ...
      for (const p in newprops.property) {
...
Object.entries(cssProperties)
    .flatMap(([jsname, cssname]) => [[jsname, cssname], [cssname, cssname]])
    .filter(([name]) => !prototype.hasOwnProperty(name))
    .map(([nameA, nameB]) => [nameA, {
      get: function () {
        return this.getPropertyValue(nameB);
      },
      set: function (value) {
        this.setProperty(nameB, value);
      }
    }])
