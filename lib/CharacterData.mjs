...
export default class CharacterData extends Leaf {
  ...
  replaceData(offset, count, data) {
    const curtext = this.data, len = curtext.length;
    // Convert arguments to correct WebIDL type
    offset = offset >>> 0;
    count = count >>> 0;
    data = String(data);

    if (offset > len || offset < 0) IndexSizeError();
