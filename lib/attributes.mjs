import {assert, IndexSizeError} from './utils.mjs';

export function property(attr) {
  if (Array.isArray(attr.type)) {
    const valid = Object.create(null);
    attr.type.forEach(function(val) {
      valid[val.value || val] = val.alias || val;
    });
    let missingValueDefault = attr.missing;
    if (missingValueDefault===undefined) { missingValueDefault = null; }
    let invalidValueDefault = attr.invalid;
...
function numberPropDesc(a) {
  ...
  if(typeof a.default === 'function') {
    ...
    def = function() { assert(false, typeof a.default); };
  }
  const unsigned_long = a.type === 'unsigned long';
  const signed_long = a.type === 'long';
  const unsigned_fallback = a.type === 'limited unsigned long with fallback';
  let min = a.min;
  let max = a.max;
  const setmin = a.setmin;
  if (min === undefined) {
  ...
  return {
    get: function() {
      const v = this._getattr(a.name);
      let n = a.float ? parseFloat(v) : parseInt(v, 10);
    ...
    set: function(v) {
      ...
      if (setmin !== undefined && v < setmin) {
        IndexSizeError(a.name + ' set to ' + v);
      }
      if (unsigned_long) {
 ...
 */
export function registerChangeHandler(c, name, handler) {
  const p = c.prototype;
